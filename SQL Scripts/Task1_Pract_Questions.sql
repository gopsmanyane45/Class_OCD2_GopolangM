...
SELECT *   FROM dbo.[property24 CSV] WHERE PROPERTY_PRICE < 1500000

---6.	List properties with more than 3 bedrooms
SELECT *   FROM dbo.[property24 CSV] WHERE BEDROOMS > 3

----7.	Find properties with parking for at least 2 cars.
SELECT *  FROM dbo.[property24 CSV] WHERE PARKING >= 2

---8.	Show properties where the monthly repayment is greater than R25,000.
SELECT *   FROM dbo.[property24 CSV] WHERE Monthly_Repayment >= 25000

---9.	Show all properties ordered by property price from highest to lowest
SELECT *  FROM dbo.[property24 CSV] ORDER BY PROPERTY_PRICE DESC 

--10.	List properties ordered by floor size from smallest to largest
SELECT *  FROM dbo.[property24 CSV] ORDER BY FLOOR_SIZE ASC 

--11.	Show Gauteng properties ordered by monthly repayment
SELECT *  FROM dbo.[property24 CSV] WHERE PROVINCE = 'Gauteng' ORDER BY Monthly_Repayment

--12.	Find Western Cape properties priced below R3,000,000
SELECT  *  FROM dbo.[property24 CSV] WHERE PROVINCE = 'Western Cape' AND  PROPERTY_PRICE < 3000000

--13.	Show KwaZulu-Natal properties with 3 or more bedrooms
SELECT  *  FROM dbo.[property24 CSV] WHERE PROVINCE = 'KwaZulu-Natal' AND  BEDROOMS >= 3

--14.	Find properties in Limpopo or Free State ordered by property price.
SELECT  *  FROM dbo.[property24 CSV] WHERE PROVINCE IN( 'Limpopo', 'Free State') ORDER BY PROPERTY_PRICE ASC ---going up

SELECT  *  FROM dbo.[property24 CSV] WHERE PROVINCE = 'Limpopo' OR  PROVINCE = 'Free State'ORDER BY PROPERTY_PRICE 
...
SELECT TOP 5 *  FROM dbo.[property24 CSV] ORDER BY PROPERTY_PRICE ASC
...
SELECT  PROVINCE AS Highest_Priced_Property, MAX(PROPERTY_PRICE)  FROM dbo.[property24 CSV] GROUP BY PROVINCE ORDER BY MAX(PROPERTY_PRICE) DESC ---NC
...
SELECT TOP 1 PROVINCE,MAX(PROPERTY_PRICE) AS HIGHEST_PRICE  FROM dbo.[property24 CSV] GROUP BY PROVINCE ORDER BY HIGHEST_PRICE DESC
...
SELECT  CITY,MIN(PROPERTY_PRICE)  FROM dbo.[property24 CSV] GROUP BY CITY ORDER BY MIN(PROPERTY_PRICE) ASC ----Tzaneen 

--20	What minimum income is typically required for properties priced above R4,000,000 

SELECT MIN(Min_Gross_Monthly_Income) AS MIN_INCOME_REQUIRED, PROVINCE FROM dbo.[property24 CSV] WHERE Min_Gross_Monthly_Income ORDER BY PROVINCE

SELECT top 1 *   FROM dbo.[property24 CSV] WHERE PROPERTY_PRICE > 4000000 ORDER BY PROPERTY_PRICE
